import Link from "next/link"
import { Heart, Instagram, Phone, Mail } from "lucide-react"

const Facebook = ({ size }: { size?: number }) => (
  <svg width={size || 18} height={size || 18} viewBox="0 0 24 24" fill="currentColor">
    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
  </svg>
)

const TikTok = ({ size }: { size?: number }) => (
  <svg width={size || 18} height={size || 18} viewBox="0 0 24 24" fill="currentColor">
    <path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z"/>
  </svg>
)

export function Footer() {
  const currentYear = new Date().getFullYear()

  return (
    <footer className="bg-foreground text-background py-12 md:py-16">
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 md:gap-12 mb-10 md:mb-12">
          {/* Brand */}
          <div>
            <Link href="#inicio" className="inline-block mb-4">
              <span className="font-serif text-2xl font-bold">Jenny Bautista</span>
            </Link>
            <p className="text-background/70 text-sm leading-relaxed mb-6">
              Psicoterapeuta Conductual dedicada a transformar vidas a traves del 
              equilibrio emocional y el desarrollo de habitos saludables.
            </p>
            <div className="flex gap-3">
              <a
                href="https://www.instagram.com/jennybautista_oficial/"
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 bg-background/10 rounded-full flex items-center justify-center hover:bg-background/20 transition-colors"
                aria-label="Instagram"
              >
                <Instagram size={18} />
              </a>
              <a
                href="https://www.facebook.com/jenny.bautista.garcia.2025?locale=es_LA"
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 bg-background/10 rounded-full flex items-center justify-center hover:bg-background/20 transition-colors"
                aria-label="Facebook"
              >
                <Facebook size={18} />
              </a>
              <a
                href="https://www.tiktok.com/@jennybautistagarcia"
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 bg-background/10 rounded-full flex items-center justify-center hover:bg-background/20 transition-colors"
                aria-label="TikTok"
              >
                <TikTok size={18} />
              </a>
              <a
                href="tel:+573106139879"
                className="w-10 h-10 bg-background/10 rounded-full flex items-center justify-center hover:bg-background/20 transition-colors"
                aria-label="Telefono"
              >
                <Phone size={18} />
              </a>
            </div>
          </div>

          {/* Quick Links */}
          <div className="sm:col-span-1">
            <h4 className="font-medium text-lg mb-4">Enlaces Rapidos</h4>
            <nav className="flex flex-col gap-3">
              {[
                { href: "#sobre-mi", label: "Sobre Mi" },
                { href: "#servicios", label: "Servicios" },
                { href: "#capacitaciones", label: "Capacitaciones" },
                { href: "#precios", label: "Precios" },
                { href: "#contacto", label: "Contacto" },
              ].map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="text-background/70 hover:text-background transition-colors text-sm"
                >
                  {link.label}
                </Link>
              ))}
            </nav>
          </div>

          {/* Contact */}
          <div>
            <h4 className="font-medium text-lg mb-4">Contacto</h4>
            <div className="space-y-3 text-sm text-background/70">
              <p className="flex items-center gap-2">
                <Phone size={14} className="text-background/50" />
                <a href="tel:+573106139879" className="hover:text-background transition-colors">
                  +57 310 613 9879
                </a>
              </p>
              <p className="flex items-center gap-2">
                <Mail size={14} className="text-background/50" />
                <a href="mailto:jennybautista28@hotmail.com" className="hover:text-background transition-colors">
                  jennybautista28@hotmail.com
                </a>
              </p>
              <p className="flex items-center gap-2">
                <Instagram size={14} className="text-background/50" />
                <a 
                  href="https://www.instagram.com/jennybautista_oficial/" 
                  target="_blank" 
                  rel="noopener noreferrer"
                  className="hover:text-background transition-colors"
                >
                  @jennybautista_oficial
                </a>
              </p>
            </div>
          </div>
        </div>

        {/* Bottom Bar */}
        <div className="pt-6 md:pt-8 border-t border-background/10 flex flex-col sm:flex-row justify-between items-center gap-3 md:gap-4 text-center sm:text-left">
          <p className="text-sm text-background/60">
            {currentYear} Jenny Bautista Garcia. Todos los derechos reservados.
          </p>
          <p className="text-sm text-background/60 flex items-center gap-1">
            Hecho con <Heart size={14} className="text-accent" fill="currentColor" /> en Colombia
          </p>
        </div>
      </div>
    </footer>
  )
}
